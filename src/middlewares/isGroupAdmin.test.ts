...
describe('isGroupAdmin', () => {
  ...
  it('deve retornar 401 quando user_id não está presente na requisição', async () => {
    const req = makeReq({ user_id: undefined, params: { id: 'g-1' } }) as any;
    const res = makeRes();

    await isGroupAdmin(req, res as any, next);
  ...
  it('deve retornar 403 quando o usuário não é membro do grupo', async () => {
    ...
    const req = makeReq({ user_id: 'u-1', params: { id: 'g-1' } }) as any;
    const res = makeRes();
  ...
  it('deve retornar 403 quando o usuário é membro mas tem role STUDENT', async () => {
  ...
  it('deve chamar next() quando o usuário tem role ADMIN', async () => {
